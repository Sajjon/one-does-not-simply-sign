    ...
    pub async fn use_factor_sources(
        ...
        match &self.driver {
            ...
            UseFactorSourceDriver::ParallelBatch(driver) => {
                ...
                let request = collector.request_for_parallel_batch_driver(
            ...
            UseFactorSourceDriver::SerialBatch(driver) => {
                for factor_source in factor_sources {
                    // Prepare the request for the driver
                    let request = collector.request_for_serial_batch_driver(&factor_source.id);
            ...
            UseFactorSourceDriver::SerialSingle(driver) => {
                ...
                'loop_0: for factor_source in factor_sources.clone() {
                    let requests_per_transaction =
                        collector.requests_for_serial_single_driver(&factor_source.id);
                ...
                // Report the results back to the collector, as a batch response
                collector.process_batch_response(SignWithFactorSourceOrSourcesOutcome::Signed {
                    produced_signatures: BatchSigningResponse::new(per_factor_source_outputs),
                });
                collector.process_batch_response(SignWithFactorSourceOrSourcesOutcome::Skipped {
